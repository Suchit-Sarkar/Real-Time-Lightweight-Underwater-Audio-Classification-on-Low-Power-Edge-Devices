...
class MarineMammalDataset(Dataset):
    ...
    def _process_audio(self, signal, sr):
        # Apply bandpass filter
        filtered_audio = self._bandpass_filter(signal, sr, lowcut=1000, highcut=24000)
        return filtered_audio
    
    def _bandpass_filter(self, audio, sr, lowcut=1000, highcut=24000, order=4):
        try:
            nyquist = 0.5 * sr
            low = lowcut / nyquist
...
if __name__ == "__main__":
    ...
    mel_spectrogram = torchaudio.transforms.MelSpectrogram(
        ...
        n_fft=n_fft,  # Smaller window size for VGGish
        hop_length=hop_length,  # Matches VGGish expectation
        n_mels=n_mels
    )
 ...
 #Create the sequential layer 
    ...
    model.fc=create_fc(12288+num_additional_features,num_hidden_layers, neurons_per_layer,20,0.2).to(device)
    ...
    best_model=model
    best_model_path = "E:/Project_Experiments/only_dolphin_experiment/fine_tuned_marine_vggish_combined_with_noise_high_freq.pth"
...
# Training loop with validation
    for epoch in range(num_epochs):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            print(f"Best model saved with Val Accuracy: {best_val_accuracy:.4f} at epoch {epoch + 1}")
...
model_save_path = "E:/Project_Experiments/only_dolphin_experiment/fine_tuned_marine_vggish_combined_with_noise_high_freq.pth"
